...
class GradientPenalizer :
    ...
    def calculate(self, D, real, fake):
        ...
        #real = torch.reshape(real, (real.size(0), real.size(1))) # todo : fix for old curves dataset
...
class DiscEcal(nn.Module):
    def __init__(self, type, hyper, problem):
        ...
        self.main = nn.Sequential(
             ...
             nn.Conv2d(problem.nf, nfx2, kernel_size, stride, 1, bias=False),  # input is 2 x 1 x 1, output of 4x1x1
             nn.LeakyReLU(0.2),
             nn.Conv2d(nfx2, nfx4, kernel_size, stride, 1, bias=False),  # input is 2 x 1 x 1, output of 4x1x1
             nn.LeakyReLU(0.2),
             nn.Conv2d(nfx4, 1, kernel_size, 1, 0, bias=False)  # output an answers real/fake
        )
...
################################################################
#
class DiscEcalLinear(nn.Module):
    def __init__(self, type, hyper, problem):
        super(DiscEcalLinear, self).__init__()
        self.ngpu = hyper.ngpu
        self.type = type
        nfx2 = problem.nf * 2
        nfx4 = problem.nf * 4
        ...
        stride = 1
        self.main = nn.Sequential(
             nn.Conv2d(problem.nc, nfx2, kernel_size, stride, 1, bias=False),  # input is (batch_size x) 1 x 30 x 30, output of 4x1x1
             nn.LeakyReLU(0.2),
             nn.Conv2d(nfx2, nfx4, kernel_size, stride, 1, bias=False),  # input is 2 x 1 x 1, output of 4x1x1
             nn.LeakyReLU(0.2),
...
class GenEcalLinear(nn.Module):
    def __init__(self, type, hyper, problem):
        super(GenEcalLinear, self).__init__()
        self.ngpu = hyper.ngpu
        kernel_size = 3
        stride = 1
        self.nfx4 = problem.nf * 4
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(self.nfx2, problem.nf, kernel_size, stride, 1, bias=False), nn.BatchNorm2d(problem.nf),
            nn.ReLU(),
            nn.ConvTranspose2d(problem.nf, problem.nc, 1, stride, 0, bias=False),
            nn.Tanh()
        )
